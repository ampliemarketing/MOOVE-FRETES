...
export function TermsOfUseModal({ 
  ...
  const [termsContent] = useState<string>(TERMS_CONTENT);
  ...
  const formatMarkdown = (markdown: string) => {
    return markdown
      ...
      .map((line, index) => {
        ...
        if (line.match(/^[A-Z\s]+$/) && line.length > 10 && line.length < 100) {
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-neutral-200 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="font-medium">Termos e Condições de Uso</DialogTitle>
                <DialogDescription className="text-neutral-500">
                  MaisFrete - Plataforma de Logística de Fretes
                </DialogDescription>
              </div>
            </div>
            <Button
